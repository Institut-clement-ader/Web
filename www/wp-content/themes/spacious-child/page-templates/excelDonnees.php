    ...
    foreach ($users as $user) {
      ...
      if ($user->display_user == 1) {
        ...
        if ($user->status === 'Doctorant') {
          ...
          if ((($user->groupe_primaire === $groupes[$j]) && (($user->groupe_secondaire === $groupes[$j]) || ($user->groupe_secondaire === '') || ($user->groupe_secondaire === 'NULL'))) && (in_array($user->tablissement_de_rattachement, $ecoles[$i])))
            $nbdocstd++;

          //Doctorant intergroupe
          if (((($user->groupe_primaire === $groupes[$j]) xor ($user->groupe_secondaire === $groupes[$j])) && (($user->groupe_secondaire != '') && $user->groupe_secondaire != 'NULL')) && (in_array($user->tablissement_de_rattachement, $ecoles[$i])))
            $nbdocinter++;
        } else {

          //Permanent
          if ((in_array($user->tablissement_de_rattachement, $ecoles[$i])) && !(in_array($user->status, $grpStatut['C']) || ($user->status === 'Doctorant')) && ($user->groupe_primaire === $groupes[$j]))
            $nbperm++;

          //Membre Associé
          if ((in_array($user->tablissement_de_rattachement, $ecoles[$i])) && (in_array($user->status, $grpStatut['C'])) && $user->groupe_primaire === $groupes[$j])
            $nbautre++;
        }
      }
    }
...
$tabbudget = array(
  array('Etablissement', 'MS2M', 'MSC', 'SUMO', 'MICS'),
  array('IMT Mines Albi'),
  array('ISAE-SUPAERO'),
  array('UPS'),
  array('INSA de Toulouse'),
