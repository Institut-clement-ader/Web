...
<div class="liste">
    ...
    <ul>
        <li><?=TXT_NOM_UTI_ADD?> <div class="valeur"> <?= $event['nom_utilisateur'] ?></div> </li> </br>
        ...
        <?php if ($event['encadrant']!= NULL):  ?>
            <li><?=TXT_ENCADRANT_SEUL?></br> <div class="valeur"><?= $event['encadrant'] ?> </div> </li></br>
        <?php endif; ?>
        <!-- Si la description est null (car pas obligatoire) alors enlève cette rubrique de la liste --> 
        <?php if ($event['description']!= NULL):  ?>
            <li><?=TXT_DESC?> </br> <div class="valeur"><?= $event['description'] ?></div>  </li></br>
        <?php endif; ?>
    </ul>
</div>
<?php  
