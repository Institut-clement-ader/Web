...
class GestionBdd
{
  ...
  {
    $req = $this->bdd->prepare("SELECT COUNT(*) FROM wp_pods_these WHERE (id IN (SELECT item_id FROM wp_podsrel WHERE pod_id = 862 AND field_id=1380)) AND (date_soutenance IS NULL OR date_soutenance >= CURDATE()) AND date_debut <= CURDATE() AND fin_previsionnelle !='Arrêt'");
    $req->execute();
    $req = $req->fetchColumn();
    return $req;
  }
  ...
  {
    $req = $this->bdd->prepare('UPDATE wp_temp_zrr SET necessite_zrr = 0, num_dossier = "vide" WHERE id = ?');
    $req->execute(array($id));
    return true;
  }
