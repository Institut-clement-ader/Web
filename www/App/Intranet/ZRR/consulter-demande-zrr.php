...
<table>
  ...
  ?>
      <tbody>
        <tr>
          ...
          <?php if ($row['necessite_zrr'] == 0 && $row['num_dossier'] == "vide") { ?><td><?php echo strtoupper($row['nom']); ?></td>
            <td><?php echo ucfirst($row['prenom']); ?></td>
            <td><?php echo $username; ?></td>
            ...
            <td>
              <form id="updateNumDossier" method="POST" action="">
                ...
                <input type="hidden" name="num_dossier" value="<?= $row['num_dossier'] ?>">
                <input type="hidden" name="etablissement_accueil" value="<?= $row['etablissement_accueil'] ?>">
          ...
          <?php } else if ($row['necessite_zrr'] == 0 && $row['num_dossier'] != "vide") { ?><td><?php echo strtoupper($row['nom']); ?></td>
            <td><?php echo ucfirst($row['prenom']); ?></td>
            <td><?php echo $username; ?></td>
            <td id="accepterDemande">
              <form action="<?= site_url(); ?>/demandes-zrr/" method="POST"><button style="background-color:green" type="hidden" name="accepter" value="<?php echo $row['id']; ?>">accepter</button></form>
  ...
  if (isset($_POST['id_zrr'])) {
    ...
    wp_mail($zrr['mail'], 'ZRR : Numéro de dossier', 'Bonjour,
        ...
        Votre demande d\'accès ZRR pour ' . ucfirst($last_name) . ' ' . ucfirst($first_name) . '(id : ' . $idZrr . ') porte le numéro ' . $numDossier . ' . La réponse arrivera dans un délais de deux mois.
        
        Cordialement.', 'Bonjour,');

    header('Location: ' . site_url() . '/demandes-zrr/');

    wp_mail("tmangear@insa-toulouse.fr", 'ZRR : Numéro de dossier', 'Bonjour,
  ...
  if (isset($_POST['accepter'])) {
    ...
    wp_mail($row['mail'], 'Demande ZRR acceptée', "Bonjour,
    
      La demande d'accès à l'Institut Clément Ader est acceptée pour :  " . $row['prenom'] . ' ' . $row['nom'] . "
      
      Le responsable ou son adjoint
      ZRR Institut Clement Ader site de Toulouse
      
      N Laurien", 'Bonjour,');
    wp_mail($multiple_recipients, $row['etablissement_accueil'] . ' - ' . $row['prenom'] . ' ' . $row['nom'] .  ' : Demande ZRR acceptée', 'Bonjour,
