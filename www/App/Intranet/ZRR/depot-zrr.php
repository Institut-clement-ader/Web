 ...
 * les listes doivent pointés vers une table dans la BDD
 * 
 * 
 */
...
if (isset($_POST['valider'])) {
  ...
  $url_file = WP_CONTENT_DIR . "/uploads/zrr/" . strtolower($_POST['nom']) . strtolower($_POST['prenom']) . $user_id . ".zip";
  $new_ask = false;
  $echec_file = false;
  $valider = true;
  ...
  if (filesize($_FILES['fichier']['tmp_name']) < 2097152) {
    if ($id_ask != null && $id_ask != "0" && $id_ask != "" && $id_ask != "/") {
      $new_ask = false;
    } else {
      $new_ask = true;
    }
    $result_move_file = move_uploaded_file($_FILES['fichier']['tmp_name'], $url_file);

    if ($result_move_file == true) {
      if ($new_ask == false) {
        $req = $bdd->resetDossier($id_ask);
        wp_mail('acces_zrr_ica@insa-toulouse.fr', $etablissement_accueil . ' - ' . $_POST['prenom'] . ' ' . $_POST['nom'] . ' - Mise à jour de fichier ZRR', $name . ' a mis le fichier ZRR de ' . $_POST['prenom'] . ' ' . $_POST['nom'] . ' à jour', 'Bonjour,', array($url_file));
        wp_mail('tmangear@insa-toulouse.fr', $etablissement_accueil . ' - ' . $_POST['prenom'] . ' ' . $_POST['nom'] . ' - Mise à jour de fichier ZRR', $name . ' a mis le fichier ZRR id (id_' . $id_ask . '_' . $_POST['id_ask'] . 'de ' . $_POST['prenom'] . ' ' . $_POST['nom'] . ' à jour', 'Bonjour,', array($url_file));
      } else {
        $req = $bdd->ajouterDemande(strtolower($_POST['nom']), strtolower($_POST['prenom']), $mailArrivant, $mail, $url_file, $date_fin, $tuteur, $date_arrivee, $statut_arrivant, $etablissement_accueil);
        wp_mail('acces_zrr_ica@insa-toulouse.fr', $etablissement_accueil . ' - ' . $_POST['prenom'] . ' ' . $_POST['nom'] . ' - Nouvelle demande ZRR', $name . ' a fait une demande ZRR pour ' . $_POST['prenom'] . ' ' . $_POST['nom'] . ' : ' . site_url() . '/demandes-zrr/. La fin de mission est estimée à ' . $_POST['date_fin'], 'Bonjour,', array($url_file));
        wp_mail('tmangear@insa-toulouse.fr', $etablissement_accueil . ' - ' . $_POST['prenom'] . ' ' . $_POST['nom'] . ' - Nouvelle demande ZRR', $name . ' a fait une demande ZRR pour ' . $_POST['prenom'] . ' ' . $_POST['nom'] . ' : ' . site_url() . '/demandes-zrr/. La fin de mission est estimée à ' . $_POST['date_fin'], 'Bonjour,', array($url_file));
      }
    } else {
      $echec_file = true;
    }
...
<form id="inscription4" name="zrr" method="post" action="<?= site_url(); ?>/documents/zrr-site-de-toulouse/depot-dossier-zrr/" enctype="multipart/form-data">
  ...
  <option value="Université Toulouse Paul Sabatier"> Université de Toulouse</option>
  <option value="IMT Mines Albi"> IMT Mines ALbi</option>
  <option value="CNRS"> CNRS</option>
  <option value="CNRS"> UTTOP</option>
  </select><br /><br />
